...
export function usePlasmaBackground(containerRef: Ref<HTMLElement | undefined>) {
  ...
  const cameraBasePosition = new THREE.Vector3(0, 5, 15);
  const cameraTargetPosition = new THREE.Vector3(0, 5, 15);

  // Целевое вращение камеры для гироскопа
  const cameraTargetRotation = new THREE.Euler(0, 0, 0, 'YXZ');
  const cameraCurrentRotation = new THREE.Euler(0, 0, 0, 'YXZ');
  ...
  const updateCameraForGyro = () => {
    if (!isGyroInitialized || !PLASMA_CONFIG.enableGyroParallax || !isMobileDevice)
      return;
  ...
  const createPlasmaParticles = () => {
    const { particleCount } = PLASMA_CONFIG;
    ...
    for (let i = 0; i < particleCount; i++) {
      ...
      if (isFrontZone) {
        ...
        const _zBias = 0.6 + Math.random() * 0.4;
      ...
      sizes[i] =
        PLASMA_CONFIG.particleSize * (0.7 + Math.random() * 1.0) * sizeMultiplier;
