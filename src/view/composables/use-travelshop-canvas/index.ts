...
export function useTravelshopCanvas(containerRef: Ref<HTMLElement | undefined>) {
  ...
  // 7. Отслеживание видимости элемента
  const { initVisibilityObserver, cleanup: cleanupVisibility } = useVisibility(
    containerRef,
    {
      rootMargin: '50px', // Предзагрузка перед появлением
      threshold: 0.01,
      callOnInitIfHidden: true,
    },
  );
  ...
  const animate = () => {
    // Если анимация на паузе (элемент не виден), не обновляем, но продолжаем цикл
    // Это позволяет быстро возобновить анимацию при появлении элемента в viewport
  ...
  // Остановка анимации (полная остановка)
  const stopAnimation = () => {
  ...
  // Пауза анимации (временная остановка при скрытии из viewport)
  ...
  onMounted(async () => {
    ...
    // Инициализируем отслеживание видимости
    initVisibilityObserver(resumeAnimation, pauseAnimation);
  });
  ...
  onUnmounted(() => {
    ...
    // Очищаем отслеживание видимости
    cleanupVisibility();
