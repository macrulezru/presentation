...
export class GetRandomProductCommand {
  ...
  async execute(): Promise<ProductModel> {
    try {
      const response: ApiResponse<ProductModel> = await this.httpClient.get<ProductModel>(this.endpoint);
  ...
  async executeWithCancel(key: string): Promise<ProductModel> {
    const response = await this.httpClient.cancellableRequest<ProductModel>(key, this.endpoint);
