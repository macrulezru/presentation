...
export class GetRandomJokeCommand {
  ...
  async execute(): Promise<JokeModel> {
    try {
      const response: ApiResponse<JokeModel> = await this.httpClient.get<JokeModel>(this.endpoint);
  ...
  async executeWithCancel(key: string): Promise<JokeModel> {
    const response = await this.httpClient.cancellableRequest<JokeModel>(key, this.endpoint);
